#!/bin/sh
# Takes the preview pictures shown on the design picker (previews/*.jpg). Run ./build.sh after.
...
for page in original design-a design-b; do
  ...
  sips -s format jpeg -s formatOptions 72 --resampleWidth 960 "/tmp/saree-preview-$page.png" --out "previews/$page.jpg" >/dev/null
  rm -f "/tmp/saree-preview-$page.png"
  echo "Saved previews/$page.jpg"
done
